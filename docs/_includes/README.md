...
```bash
docker build -f build-push/Dockerfile -t jeffdecola/hello-go-deploy-amazon-ecs .
```
...
RUN go get -d -v
RUN go build -o /go/bin/hello-go-deploy-amazon-ecs main.go
```
...
![IMAGE - hello-go-deploy-amazon-ecs concourse ci pipeline - IMAGE](pics/hello-go-deploy-amazon-ecs-pipeline.jpg)
